        ...
        <div className="grid gap-10 py-10 lg:grid-cols-[1fr_360px] lg:gap-16 lg:py-14">
          <div>
            ...
            <p className="mt-5 border-b border-ink/10 pb-7 text-sm font-semibold text-ink/65">
              {stay.guests} guests &middot; {stay.bedrooms}{" "}
              {stay.bedrooms === 1 ? "bedroom" : "bedrooms"} &middot; &#9733;{" "}
              {stay.rating}
            </p>
            <p className="mt-7 max-w-3xl text-base leading-8 text-ink/70">
              {stay.longDescription}
            </p>
            <h2 className="mt-10 text-xl font-bold">What this place offers</h2>
            <ul className="mt-5 grid gap-3 sm:grid-cols-2">
              {stay.amenities.map((amenity) => (
                <li
                  key={amenity}
                  className="rounded-2xl border border-ink/10 bg-white/50 px-4 py-3 text-sm font-semibold"
                >
                  {amenity}
                </li>
              ))}
          ...
          <aside className="h-fit rounded-3xl bg-white p-6 shadow-soft lg:sticky lg:top-28">
            ...
            <div className="mt-6 rounded-2xl bg-cream p-4 text-sm leading-6 text-ink/65">
              Booking will be available in a future release. For now, enjoy
              exploring the collection.
            </div>
            <Link
              href="/stays"
              className="mt-5 block rounded-full bg-ink px-6 py-3.5 text-center text-sm font-bold text-white transition hover:bg-moss"
            >
