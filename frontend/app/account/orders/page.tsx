...
export default function AccountOrdersPage() {
  ...
  const loadOrders = useCallback(async () => {
    ...
    try {
      ...
      setError(caught instanceof Error ? caught.message : "دریافت سفارش‌ها انجام نشد.");
    } finally {
      setLoading(false);
    }
  }, [page]);
  ...
  useEffect(() => {
    ...
    if (!auth.authenticated) {
      router.replace(`/login?returnTo=${encodeURIComponent("/account/orders")}`);
      return;
    }
  ...
  if (auth.loading || !auth.authenticated || !hasAccountWorkspace) {
    return (
      <div className="grid min-h-[50vh] place-items-center px-5 text-sm font-semibold text-muted-foreground" role="status">
  ...
  return (
    <main className="min-h-screen bg-background px-4 py-6 text-foreground sm:px-6 lg:px-8" dir="rtl">
      <div className="mx-auto grid max-w-6xl gap-5">
        <KoochPageHeader
          actions={
            <div className="flex flex-wrap gap-2">
              <KoochButton onClick={() => router.push("/account/reservations")} variant="outline">رزروهای مستقل</KoochButton>
              <KoochButton onClick={() => router.push("/account")} variant="ghost">حساب کاربری</KoochButton>
            </div>
          }
        ...
        {loading ? (
          <KoochCard className="py-12 text-center text-sm font-semibold text-muted-foreground" role="status">در حال بارگذاری سفارش‌ها...</KoochCard>
        ) : orders.length === 0 ? (
          <KoochCard className="grid justify-items-center gap-3 py-12 text-center">
            <h2 className="text-lg font-black">هنوز سفارشی ثبت نشده است</h2>
            <p className="max-w-xl text-sm leading-7 text-muted-foreground">پس از انتخاب و ثبت یک یا چند اتاق، سفارش شما در این بخش نمایش داده می‌شود.</p>
            <Link className="font-semibold text-primary hover:underline" href="/">مشاهده اقامتگاه‌ها</Link>
          </KoochCard>
        ) : (
        ...
        {totalPages > 1 && (
          <nav aria-label="صفحه‌بندی سفارش‌ها" className="flex flex-wrap items-center justify-end gap-3 text-sm text-muted-foreground">
            <span>صفحه {page.toLocaleString("fa-IR")} از {totalPages.toLocaleString("fa-IR")}</span>
            <div className="flex gap-2">
              <KoochButton disabled={page <= 1 || loading} onClick={() => setPage((value) => value - 1)} size="sm" variant="outline">قبلی</KoochButton>
              <KoochButton disabled={page >= totalPages || loading} onClick={() => setPage((value) => value + 1)} size="sm" variant="outline">بعدی</KoochButton>
            </div>
          </nav>
        )}
      </div>
    ...
    <KoochCard className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-center">
      <div className="grid min-w-0 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <div className="min-w-0">
          ...
          <Link className="mt-1 block truncate font-black text-primary hover:underline" dir="ltr" href={`/account/booking-sessions/${encodeURIComponent(order.sessionCode)}`}>{order.sessionCode}</Link>
        </div>
        <div className="min-w-0">
          <p className="text-xs font-bold text-muted-foreground">اقامتگاه و تاریخ</p>
          <p className="mt-1 truncate text-sm font-bold">{order.property.name}</p>
        ...
        <div className="flex flex-wrap content-start gap-2">
          <KoochBadge variant={orderStatusVariant(order.derivedStatus)}>{orderStatusLabels[order.derivedStatus] ?? "وضعیت ترکیبی"}</KoochBadge>
          {order.paymentStatus && <KoochBadge variant={order.paymentStatus === "Successful" ? "success" : order.paymentStatus === "Failed" ? "destructive" : "muted"}>{paymentStatusLabels[order.paymentStatus] ?? order.paymentStatus}</KoochBadge>}
        </div>
      </div>
      <div className="flex flex-wrap items-center gap-3 lg:justify-end">
        {order.paymentDeadlineUtc && (
          <div aria-live="off" className="min-w-36 text-xs text-muted-foreground">
