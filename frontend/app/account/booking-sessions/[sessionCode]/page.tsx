...
export default function AccountBookingSessionPage() {
  ...
  const approvalDeadlineUtc =
    session?.summary.earliestApprovalDeadlineUtc ?? null;
  ...
  async function beginPayment() {
    ...
    try {
      ...
      setError(
        caught instanceof Error ? caught.message : "شروع پرداخت انجام نشد.",
      );
    } finally {
  ...
  useEffect(() => {
    ...
    if (!auth.authenticated) {
      router.replace(
        `/login?returnTo=${encodeURIComponent(`/account/booking-sessions/${sessionCode}`)}`,
      );
      return;
    }
    let active = true;
  ...
  const acceptedReservations = session.reservations.filter((reservation) =>
    ["ApprovedAwaitingPayment", "Confirmed", "Paid"].includes(
      reservation.status,
    ),
  );
  ...
  const fullyConfirmed =
    ...
    session.reservations.every((reservation) =>
      ["Confirmed", "Paid", "Completed"].includes(reservation.status),
    );
  const fullyRejected =
    session.reservations.length > 0 &&
    session.reservations.every(
      (reservation) => reservation.status === "Rejected",
    );
  ...
  const checkInDate =
    session.summary.earliestCheckInDate ??
    session.reservations[0]?.checkInDate ??
    null;
  const checkOutDate =
    session.summary.latestCheckOutDate ??
    session.reservations[0]?.checkOutDate ??
    null;

  return (
    <main
      className="mx-auto grid max-w-5xl gap-5 px-4 pb-28 pt-8 sm:px-6 sm:pb-8"
      dir="rtl"
    >
      <KoochPageHeader
        actions={
        ...
        <KoochAlert variant="info" title="بخشی از درخواست رزرو تأیید شده است">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div className="grid gap-1">
              <span>
                برخی رزروها تأیید و برخی رد شده‌اند. می‌توانید همه رزروهای
                تأییدشده را با یک پرداخت ادامه دهید.
              </span>
              <span className="text-sm">
                امکان انتخاب یا پرداخت جداگانه رزروها وجود ندارد.
              </span>
              {paymentProviderSelector}
            </div>
      ...
      {session.summary.hasRejectedReservations &&
        ...
        session.payment?.status !== "Successful" && (
          <KoochAlert
            ...
            title={
              fullyRejected
                ? "درخواست رزرو تأیید نشد"
                : "بخشی از درخواست رزرو تأیید نشده است"
            }
          >
            {fullyRejected
      ...
      {session.summary.hasRejectedReservations &&
        session.payment?.status === "Successful" && (
          <KoochAlert variant="info" title="نتیجه این سفارش ترکیبی است">
            پرداخت رزروهای تأییدشده با موفقیت انجام شده و رزروهای ردشده برای حفظ
            سابقه سفارش نمایش داده می‌شوند.
          </KoochAlert>
        )}
      ...
      {failedPaymentStillPayable && (
        <KoochAlert variant="warning" title="تلاش قبلی پرداخت ناموفق بود">
          مهلت پرداخت هنوز فعال است و می‌توانید دوباره از همین سفارش برای پرداخت
          اقدام کنید.
        </KoochAlert>
      )}
          ...
          <div className="grid gap-4 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center">
            <div className="grid min-w-0 gap-3">
              ...
              <strong className="text-sm text-foreground">
                مبلغ قابل پرداخت:{" "}
                {formatCurrency(payableAmount, { currencyLabel })}
              </strong>
      ...
      <section aria-labelledby="stay-details-title" className="grid gap-3">
        ...
        <KoochCard className="grid gap-5">
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            <Detail
              label={session.displayCodeLabel || "کد سفارش"}
              value={<span dir="ltr">{session.sessionCode}</span>}
            />
            <Detail
              label="اقامتگاه"
              value={
                <Link
                  ...
                  href={`/properties/${session.property.slug}`}
                >
            ...
            <Detail
              label="نوع رزرو"
              value={`${bookingMode.icon} ${bookingMode.label}`}
            />
            <Detail
              label="تاریخ ورود"
              value={checkInDate ? formatBookingDate(checkInDate) : "مشخص نشده"}
            />
            <Detail
              label="تاریخ خروج"
              value={
                checkOutDate ? formatBookingDate(checkOutDate) : "مشخص نشده"
              }
            />
            <Detail
              label="مدت اقامت"
              value={`${countBookingNights(
            ...
            {session.payment && (
              <>
                <Detail
                  label="وضعیت پرداخت"
                  value={paymentStatusLabel(session.payment.status)}
                />
                {session.payment.status === "Successful" &&
                  !session.summary.hasRejectedReservations && (
                    <Detail
                      label="مبلغ پرداخت‌شده"
                      value={formatCurrency(session.payment.amount, {
                        currencyLabel,
                      })}
                    />
                  )}
          ...
          <div className="border-t border-border pt-5">
            <h3 className="text-base font-bold text-foreground">اتاق‌ها</h3>
            <ul className="mt-3 grid gap-3" aria-label="خلاصه اتاق‌های سفارش">
              {roomTypeGroups.map((group) => (
                <li
                  className="grid min-w-0 gap-2 rounded-lg bg-muted px-4 py-3 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center"
                  key={group.key}
                >
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-x-4 gap-y-1 text-sm sm:justify-end">
                    <span className="font-bold text-foreground">
                      {group.quantity.toLocaleString("fa-IR")} اتاق
                    </span>
                    <strong className="text-foreground">
                      {formatCurrency(group.amount, { currencyLabel })}
                    </strong>
                  </div>
                </li>
              ))}
      ...
      <section aria-labelledby="guest-details-title" className="grid gap-3">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
              ...
              <div className="mt-4 grid gap-3">
                ...
                {auth.user.phoneNumber && (
                  <Detail
                    ...
                    value={
                      <span dir="ltr">
                        {toPersianDigits(auth.user.phoneNumber)}
                      </span>
                    }
                  />
                )}
                {auth.user.email && (
                  <Detail
                    label="ایمیل"
                    value={<span dir="ltr">{auth.user.email}</span>}
                  />
                )}
              </div>
            </KoochCard>
          ...
          <KoochCard>
            <h3 className="text-base font-bold text-foreground">مهمان اصلی</h3>
            <div className="mt-4 grid gap-3">
              {session.primaryGuest ? (
                <>
                  <Detail
                    label="نام و نام خانوادگی"
                    value={`${session.primaryGuest.firstName} ${session.primaryGuest.lastName}`}
                  />
                  {session.primaryGuest.mobile && (
                    <Detail
                      label="شماره موبایل"
                      value={
                        <span dir="ltr">
                          {toPersianDigits(session.primaryGuest.mobile)}
                        </span>
                      }
                    />
                  )}
                  {session.primaryGuest.email && (
                    <Detail
                      label="ایمیل"
                      value={
                        <span dir="ltr">{session.primaryGuest.email}</span>
                      }
                    />
                  )}
                </>
                ...
                <p className="text-sm text-muted-foreground">
                  اطلاعات مهمان اصلی در دسترس نیست.
                </p>
              )}
            </div>
          </KoochCard>
          ...
          {session.specialRequest?.trim() && (
            <KoochCard>
              ...
              <p className="mt-4 break-words whitespace-pre-wrap text-sm leading-7 text-foreground">
      ...
      <section
        aria-labelledby="session-reservations-title"
        className="grid gap-4"
      >
        <h2 className="text-xl font-bold" id="session-reservations-title">
          رزروهای این سفارش
        </h2>
        {session.summary.hasRejectedReservations &&
        !session.summary.hasPendingApprovals ? (
      ...
      {canInitiatePayment && canRenderPaymentAction && (
        <div
          className="fixed inset-x-0 bottom-0 z-40 border-t border-border bg-background/95 p-3 shadow-lg backdrop-blur sm:hidden"
          data-testid="session-payment-mobile-action"
          dir="rtl"
        >
  ...
  return reservations.map((reservation) => (
    ...
    >
      <div className="grid min-w-0 gap-3 sm:grid-cols-2">
        <Detail
          label="شماره رزرو"
          value={<span dir="ltr">{reservation.reservationNumber}</span>}
        />
        <Detail
          label="اتاق"
          value={`${reservation.roomTypeName}${reservation.roomName ? `، ${reservation.roomName}` : ""}`}
        />
        <Detail
          label="تاریخ ورود"
          value={formatBookingDate(reservation.checkInDate)}
        />
        <Detail
          label="تاریخ خروج"
          value={formatBookingDate(reservation.checkOutDate)}
        />
      </div>
      <div className="flex min-w-36 flex-col items-start justify-between gap-3 sm:items-end">
        ...
        <strong>
          {formatCurrency(reservation.finalAmount, { currencyLabel })}
        </strong>
      </div>
    </KoochCard>
  ));
  ...
  return (
    <div className="min-w-0" aria-live="off">
      ...
      <p className="mt-1 text-xs font-bold text-primary" role="timer">
        زمان باقی‌مانده:{" "}
        {formatBookingCountdown(remainingSeconds, expiredLabel)}
      </p>
    </div>
  );
}
