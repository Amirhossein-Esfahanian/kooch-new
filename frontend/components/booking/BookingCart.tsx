...
export function BookingCartSummary({
  ...
  const mode = firstItem
    ? bookingModePresentation(firstItem.bookingMode)
    : null;
      ...
      {items.length === 0 ? (
        ...
        <>
          ...
          <div className="mt-3 border-y border-border py-3 text-sm">
            <p className="font-bold leading-7 text-foreground">
              {formatBookingDateRange(firstItem.checkIn, firstItem.checkOut)}
            </p>
            <p className="mt-1 text-muted-foreground">
              {nightsCount.toLocaleString("fa-IR")} شب ·{" "}
              {firstItem.adults.toLocaleString("fa-IR")} بزرگسال
          ...
          <dl className="mt-3 grid grid-cols-2 gap-3 text-sm">
            <SummaryValue
              label="تعداد نوع اتاق"
              value={roomTypeCount.toLocaleString("fa-IR")}
            />
            <SummaryValue
              label="تعداد اتاق"
