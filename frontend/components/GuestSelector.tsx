...
export function GuestSelector({
  ...
  const guestSummary =
    `${formatFaNumber(value.adults)} بزرگسال، ` +
    `${formatFaNumber(value.children)} کودک`;

  const roomSummary = `${formatFaNumber(value.rooms)} اتاق`;

  return (
    <div
      className={`relative grid gap-2 text-sm font-bold text-slate-700 ${className}`}
      ref={wrapperRef}
    >
      ...
      >
        ...
        <span
          aria-hidden="true"
          className={`shrink-0 text-lg text-slate-500 transition motion-reduce:transition-none ${
            open ? "rotate-180" : ""
          }`}
        >
          ⌄
