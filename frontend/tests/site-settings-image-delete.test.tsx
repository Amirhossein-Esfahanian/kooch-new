...
const settings = ["site.logoUrl", "home.heroBackgroundUrl"].map(
  (key, index) => ({
    id: index + 1,
    key,
...
function openDelete(index = 0) {
  fireEvent.click(
    screen.getAllByRole("button", { name: "گزینه‌های تصویر" })[index],
  );
...
describe("Site Settings persisted image deletion", () => {
  beforeEach(() => {
    ...
    api.mockImplementation(async (path: string, init?: RequestInit) => {
      ...
      if (init?.method === "DELETE")
        return {
          ...settings.find((item) => path.includes(item.key)),
          value: "",
        };
      return settings;
    });
  ...
  it.each(settings.map((item, index) => [item.key, index] as const))(
    ...
    async (key, index) => {
      ...
      fireEvent.click(
        within(dialog).getByRole("button", { name: "حذف تصویر" }),
      );
      await waitFor(() =>
        expect(api).toHaveBeenCalledWith(`/admin/site-settings/${key}/image`, {
          method: "DELETE",
        }),
      );
  ...
  it("enables persisted crop for the hero background while keeping logo crop disabled", async () => {
    render(<AdminSiteSettingsPage />);
    const actions = await screen.findAllByRole("button", {
      name: "گزینه‌های تصویر",
    });

    fireEvent.click(actions[0]);
    expect(screen.getByRole("menuitem", { name: "برش تصویر" })).toHaveProperty(
      "disabled",
      true,
    );
