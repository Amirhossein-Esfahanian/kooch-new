...
public interface IRoomDailyPriceService
{
    ...
    Task<IReadOnlyList<RoomDailyPriceResponse>> CopyAsync(int userId, UserRole role, int propertyId, CopyRoomDailyPriceRequest request, CancellationToken cancellationToken = default);
    Task<IReadOnlyList<RoomDailyPriceHistoryResponse>> GetHistoryAsync(int userId, UserRole role, int propertyId, PricingGuestType? guestType = null, CancellationToken cancellationToken = default);
}
